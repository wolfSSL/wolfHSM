...
int wh_Client_Ed25519Sign(whClientContext* ctx, ed25519_key* key,
                          const uint8_t* msg, uint32_t msgLen, uint8_t* sig,
                          uint32_t* inout_sig_len)
{
    ...
    uint16_t req_len =
        sizeof(whMessageCrypto_GenericRequestHeader) + sizeof(*req) + msgLen;
    if (req_len > WOLFHSM_CFG_COMM_DATA_LEN) {
        return WH_ERROR_BADARGS;
    }
...
int wh_Client_Ed25519Verify(whClientContext* ctx, ed25519_key* key,
                            ...
                            const uint8_t* msg, uint32_t msgLen, int* out_res)
{
    int                                    ret     = 0;
