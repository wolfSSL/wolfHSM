                            ...
                            void* cryptoDataOut, uint16_t* outSize)
{
    ...
    if (ret == WH_ERROR_OK && keyLen > 0) {
        ...
        ret = whServerDma_CopyFromClient(ctx, tmpKey, req.key.addr, keyLen,
                                         (whServerDmaFlags){0});
        if (ret != WH_ERROR_OK) {
    ...
    if (ret == WH_ERROR_OK && req.iv.sz > 0) {
        ...
        ret = wh_Server_DmaProcessClientAddress(
            ctx, req.iv.addr, &ivAddr, req.iv.sz,
            WH_DMA_OPER_CLIENT_WRITE_PRE, (whServerDmaFlags){0});
        if (ret != WH_ERROR_OK) {
    ...
    /* Handle tmp */
    if (ret == WH_ERROR_OK && req.tmp.sz > 0) {
        /* Process client address for tmp */
        ret = wh_Server_DmaProcessClientAddress(
            ctx, req.tmp.addr, &tmpAddr, req.tmp.sz,
            WH_DMA_OPER_CLIENT_WRITE_PRE, (whServerDmaFlags){0});
        if (ret != WH_ERROR_OK) {
            res.dmaAddrStatus.badAddr = req.tmp;
        }
    }
    ...
    if (ret == WH_ERROR_OK) {
        /* load the key */
        ret = wc_AesSetKeyDirect(aes, (byte*)tmpKey, (word32)keyLen,
                                 (byte*)ivAddr,
                                 enc != 0 ? AES_ENCRYPTION : AES_DECRYPTION);
        if (ret == 0) {
            /* do the crypto operation */
            /* restore previous left */
            aes->left = left;
            memcpy(aes->tmp, tmpAddr, sizeof(aes->tmp));
            if (enc != 0) {
            ...
            if (ret == 0) {
                left = aes->left;
                outSz = len;
                memcpy(ivAddr, aes->reg, AES_BLOCK_SIZE);
                memcpy(tmpAddr, aes->tmp, sizeof(aes->tmp));
            }
        }
    }


    /* Post-write DMA address processing for out, iv, and tmp (on success) */
    if (ret == WH_ERROR_OK) {
        if (req.output.sz > 0) {
                         ...
                         void* cryptoDataOut, uint16_t* outSize)
{
    ...
    uint64_t needed_size =
        sizeof(whMessageCrypto_AesEcbResponse) + len + key_len;
    if (needed_size > inSize) {
        return WH_ERROR_BADARGS;
    }
                            ...
                            void* cryptoDataOut, uint16_t* outSize)
{
    ...
    if (ret == WH_ERROR_OK) {
        ret = wc_AesSetKey(aes, tmpKey, keyLen, NULL,
    ...
    *outSize  = sizeof(whMessageCrypto_AesEcbDmaResponse);
    ...
    /* Translate response back */
    ret = wh_MessageCrypto_TranslateAesEcbDmaResponse(
                            ...
                            void* cryptoDataOut, uint16_t* outSize)
{
    ...
    /* Post-write DMA address processing for output and iv (on success) */
    if (ret == WH_ERROR_OK) {
        if (req.output.sz > 0) {
            int rc2 = wh_Server_DmaProcessClientAddress(
                ctx, req.output.addr, &outAddr, req.output.sz,
    ...
    *outSize  = sizeof(whMessageCrypto_AesCbcDmaResponse);
    ...
    /* Translate response back */
    ret = wh_MessageCrypto_TranslateAesCbcDmaResponse(
                            ...
                            void* cryptoDataOut, uint16_t* outSize)
{
    ...
    if (ret == WH_ERROR_OK && req.finalize) {
        ...
        if (ret == WH_ERROR_OK) {
            WH_DEBUG_SERVER_VERBOSE("  wc_Sha256Final: outAddr=%p\n", outAddr);
        ...
        if (ret == WH_ERROR_OK) {
            WH_DEBUG_SERVER_VERBOSE("  wc_Sha256Update: inAddr=%p, sz=%llu\n", inAddr,
                            ...
                            void* cryptoDataOut, uint16_t* outSize)
{
    ...
    if (ret == WH_ERROR_OK && req.finalize) {
        ...
        if (ret == WH_ERROR_OK) {
            WH_DEBUG_SERVER_VERBOSE("  wc_Sha224Final: outAddr=%p\n", outAddr);
        ...
        if (ret == WH_ERROR_OK) {
            WH_DEBUG_SERVER_VERBOSE("  wc_Sha224Update: inAddr=%p, sz=%llu\n", inAddr,
                   (long long unsigned int)req.input.sz);
                            ...
                            void* cryptoDataOut, uint16_t* outSize)
{
    ...
    if (ret == WH_ERROR_OK && req.finalize) {
        ...
        if (ret == WH_ERROR_OK) {
            WH_DEBUG_SERVER_VERBOSE("  wc_Sha384Final: outAddr=%p\n", outAddr);
        ...
        if (ret == WH_ERROR_OK) {
            WH_DEBUG_SERVER_VERBOSE("  wc_Sha384Update: inAddr=%p, sz=%llu\n", inAddr,
                   (long long unsigned int)req.input.sz);
                            ...
                            void* cryptoDataOut, uint16_t* outSize)
{
    ...
    if (ret == WH_ERROR_OK && req.finalize) {
        ...
        if (ret == WH_ERROR_OK) {
            WH_DEBUG_SERVER_VERBOSE("  wc_Sha512Final: outAddr=%p\n", outAddr);
            WH_DEBUG_SERVER_VERBOSE("  hashTpe: %d\n", hashType);
            switch (hashType) {
                case WC_HASH_TYPE_SHA512_224:
        ...
        if (ret == WH_ERROR_OK) {
            WH_DEBUG_SERVER_VERBOSE("  wc_Sha512Update: inAddr=%p, sz=%llu\n", inAddr,
                   (long long unsigned int)req.input.sz);
                          ...
                          void* cryptoDataOut, uint16_t* outSize)
{
    ...
    if (inSize < sizeof(whMessageCrypto_CmacDmaRequest)) {
        return WH_ERROR_BADARGS;
    }
