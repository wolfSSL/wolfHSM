                              ...
                              void* cryptoDataOut, uint16_t* outSize)
{
    ...
    if (ret == 0) {
        ...
        if (ret == WH_ERROR_OK) {
            uint8_t  seed[32];
            uint32_t seed_len = sizeof(seed);
            wc_ed25519_export_private_only(key, seed, &seed_len);
            WH_DEBUG_SERVER_VERBOSE(
                "Ed25519 key (%d bytes) loaded for verify\n", seed_len);
            WH_DEBUG_VERBOSE_HEXDUMP("", seed, seed_len);
            WH_DEBUG_SERVER_VERBOSE("\n");
            WH_DEBUG_VERBOSE_HEXDUMP("msg:\n", req_msg, msg_len);
            ret = wc_ed25519_sign_msg(req_msg, msg_len, sig, &sig_len, key);
            WH_DEBUG_VERBOSE_HEXDUMP("Sig:\n", sig, sig_len);
        }
        wc_ed25519_free(key);
    }
                                ...
                                void* cryptoDataOut, uint16_t* outSize)
{
    ...
    if (ret == 0) {
        ...
        if (ret == WH_ERROR_OK) {
            ...
            WH_DEBUG_VERBOSE_HEXDUMP("", seed, seed_len);
            WH_DEBUG_SERVER_VERBOSE("\n");
            WH_DEBUG_VERBOSE_HEXDUMP("msg:", req_msg, msg_len);
            WH_DEBUG_VERBOSE_HEXDUMP("sig:", req_sig, sig_len);
            ret = wc_ed25519_verify_msg(req_sig, sig_len, req_msg, msg_len,
                                        &result, key);
        }
        wc_ed25519_free(key);
    }
                                 ...
                                 void* cryptoDataOut, uint16_t* outSize)
{
    ...
    if (ret == WH_ERROR_OK) {
        ...
        if (ret == 0) {
            ret = wh_Server_Ed25519KeyCacheExport(ctx, key_id, key);
            if (ret == WH_ERROR_OK) {
                ret = wc_ed25519_sign_msg(msgAddr, req.msg.sz, sigAddr, &sigLen,
                                          key);
                if (ret == WH_ERROR_OK) {
                    res.sigSz = sigLen;
                }
                                   ...
                                   void* cryptoDataOut, uint16_t* outSize)
{
    ...
    if (ret == WH_ERROR_OK) {
        ...
        if (ret == 0) {
            ...
            if (ret == WH_ERROR_OK) {
                int verified = 0;
                ret = wc_ed25519_verify_msg(sigAddr, req.sig.sz, msgAddr,
                                            req.msg.sz, &verified, key);
                if (ret == WH_ERROR_OK) {
                            ...
                            void* cryptoDataOut, uint16_t* outSize)
{
    ...
    if (inSize < sizeof(whMessageCrypto_AesDmaRequest)) {
        return WH_ERROR_BADARGS;
    }
