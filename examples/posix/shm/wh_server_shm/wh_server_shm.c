/*
 * Example server app using POSIX shared memory transport
 */
                         ...
                         int clientId)
{
    ...
    if (ret == 0) {
        ...
        wh_Server_SetConnected(server, 1); /* start in connected state after
                                              shared memory has been setup */
        while (1) {
            ...
            if (ret == WH_ERROR_NOTREADY) {
                ...
                if (get_conn_result == WH_ERROR_OK) {
                    if (current_state == WH_COMM_CONNECTED &&
                        ...
                        if (ret == 0) {
                            wh_Server_SetConnected(server, 1);
                            /* start in connected state after shared memory has
                               been setup */
                        }
...
int main(int argc, char** argv)
{
    ...
    whTransportServerCb            ptttcb[1]      = {POSIX_TRANSPORT_SHM_SERVER_CB};
    posixTransportShmServerContext tsc[1]         = {};
