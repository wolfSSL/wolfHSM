...
static int whTest_ShowNvmAvailable(whClientContext* ctx)
{
    ...
    if (ret != 0) {
        ...
        WOLFHSM_CFG_PRINTF("CRYPTO TEST NVM STATUS: NvmGetAvailable:%d, server_rc:%d "
...
#define RSA_EXPONENT WC_RSA_EXPONENT
    ...
    if (ret == 0) {
        WOLFHSM_CFG_PRINTF("RSA SUCCESS\n");
    }
    return ret;
}
#endif /* !NO_RSA */
                    ...
                    if (ret != 0) {
                        ...
                        if (ret != 0) {
                            ...
                            if (memcmp(shared_ab, shared_ba, secLen) == 0) {
                                WOLFHSM_CFG_PRINTF("ECDH SUCCESS\n");
                            }
                            else {
                    ...
                    if (ret == 0) {
                        ...
                        if (ret != 0) {
                            ...
                            if (ret != 0) {
                                ...
                                if (res == 1) {
                                    WOLFHSM_CFG_PRINTF("ECC SIGN/VERIFY SUCCESS\n");
                                }
                                else {
...
static int whTest_CryptoCurve25519(whClientContext* ctx, int devId, WC_RNG* rng)
{
    ...
    if (ret == 0) {
        WOLFHSM_CFG_PRINTF("CURVE25519 SUCCESS\n");
    }
    return ret;
}
...
static int whTest_CryptoSha256(whClientContext* ctx, int devId, WC_RNG* rng)
{
    ...
    if (ret == 0) {
        WOLFHSM_CFG_PRINTF("SHA256 DEVID=0x%X SUCCESS\n", devId);
    }
    return ret;
}
#endif /* !NO_SHA256 */
...
static int whTest_CryptoSha224(whClientContext* ctx, int devId, WC_RNG* rng)
{
    ...
    if (ret == 0) {
        WOLFHSM_CFG_PRINTF("SHA224 DEVID=0x%X SUCCESS\n", devId);
    }
    return ret;
}
...
static int whTest_CryptoSha384(whClientContext* ctx, int devId, WC_RNG* rng)
{
    ...
    if (ret == 0) {
        WOLFHSM_CFG_PRINTF("SHA384 DEVID=0x%X SUCCESS\n", devId);
    }
    return ret;
}
...
static int whTest_CryptoSha512(whClientContext* ctx, int devId, WC_RNG* rng)
{
    ...
    if (ret == 0) {
        WOLFHSM_CFG_PRINTF("SHA512 DEVID=0x%X SUCCESS\n", devId);
    }
    return ret;
}
...
#define WH_TEST_KEYCACHE_KEYSIZE 16
    ...
    if (ret == 0) {
        ...
        if (ret != 0) {
            ...
            WOLFHSM_CFG_PRINTF("KEY CACHE/EXPORT SUCCESS\n");
        }
    }
#ifdef WOLFHSM_CFG_IS_TEST_SERVER
                ...
                if (ret != 0) {
                    ...
                    if (ret != 0) {
                        ...
                        if ((outLen != sizeof(key)) ||
                            ...
                            WOLFHSM_CFG_PRINTF("KEY CACHE USER EXCLUSION SUCCESS\n");
                        }
                    }
                }
            }
    ...
    if (ret == 0) {
        ...
        if (ret != 0) {
            ...
            if (ret != WH_ERROR_NOTFOUND) {
                ...
                WOLFHSM_CFG_PRINTF("KEY CACHE EVICT SUCCESS\n");
                ret = 0;
            }
        }
    }
    ...
    if (ret == 0) {
        ...
        if (ret == 0) {
            WOLFHSM_CFG_PRINTF("KEY COMMIT/ERASE SUCCESS\n");
        }
    }
    ...
    if (ret == 0) {
        ...
        if (ret == 0) {
            WOLFHSM_CFG_PRINTF("KEY CROSS-CACHE EVICTION AND REPLACEMENT SUCCESS\n");
        }
    }
    ...
    if (ret == 0) {
        ...
        if (ret != WH_ERROR_OK) {
            ...
            if (ret != 0) {
                ...
                WOLFHSM_CFG_PRINTF("KEY CACHE/EXPORT DMA SUCCESS\n");
            }
        }
    }
    ...
    if (ret == 0) {
        ...
        if (ret == 0) {
            WOLFHSM_CFG_PRINTF("KEY CROSS-CACHE EVICTION AND REPLACEMENT DMA SUCCESS\n");
        }
    }
#endif /* WOLFHSM_CFG_DMA */
                                        ...
                                        WC_RNG* rng)
{
    ...
    WOLFHSM_CFG_PRINTF("Testing non-exportable keystore enforcement...\n");
    ...
    WOLFHSM_CFG_PRINTF("Non-exportable key export correctly denied\n");
    ...
    WOLFHSM_CFG_PRINTF("Exportable key export succeeded\n");
    ...
    WOLFHSM_CFG_PRINTF("Testing DMA key export protection...\n");
    ...
    WOLFHSM_CFG_PRINTF("Non-exportable key DMA export correctly denied\n");
    ...
    WOLFHSM_CFG_PRINTF("Exportable key DMA export succeeded\n");

    /* Clean up */
    wh_Client_KeyEvict(ctx, keyId);
#endif /* WOLFHSM_CFG_DMA */

    WOLFHSM_CFG_PRINTF("NON-EXPORTABLE KEYSTORE TEST SUCCESS\n");
    return 0;
}
...
#define WH_TEST_AES_TEXTSIZE 16
    ...
    if (ret == 0) {
        ...
        if (ret == 0) {
            WOLFHSM_CFG_PRINTF("AES CTR DEVID=0x%X SUCCESS\n", devId);
        }
    }
#endif
    ...
    if (ret == 0) {
        ...
        if (ret == 0) {
            WOLFHSM_CFG_PRINTF("AES ECB DEVID=0x%X SUCCESS\n", devId);
        }
    }
#endif /* HAVE_AES_ECB */
    ...
    if (ret == 0
        ...
        if (ret == 0) {
            WOLFHSM_CFG_PRINTF("AES CBC DEVID=0x%X SUCCESS\n", devId);
        }
    }
#endif /* HAVE_AES_CBC */
...
#define WH_TEST_AES_TAGSIZE 16
    ...
    if (ret == 0) {
        ...
        if (ret == 0) {
            WOLFHSM_CFG_PRINTF("AES GCM DEVID=0x%X SUCCESS\n", devId);
        }
    }
#endif /* HAVE_AES_GCM */
    return ret;
...
#define WH_TEST_CMAC_TEXTSIZE 1000
    ...
    if (ret == 0) {
        WOLFHSM_CFG_PRINTF("CMAC DEVID=0x%X SUCCESS\n", devId);
    }
    return ret;
}
                                       ...
                                       WC_RNG* rng)
{
    ...
    if (ret == 0) {
        ...
        if (ret != 0) {
            ...
            WOLFHSM_CFG_PRINTF("ML-DSA DEVID=0x%X SUCCESS\n", devId);
        }
    }
                                       ...
                                       WC_RNG* rng)
{
    ...
    if (ret == 0) {
        WOLFHSM_CFG_PRINTF("ML-DSA Client DMA API SUCCESS\n");
    }
                                    ...
                                    WC_RNG* rng)
{
    ...
    if (ret == WH_ERROR_OK) {
        WOLFHSM_CFG_PRINTF("ML-DSA VERIFY ONLY: SUCCESS\n");
    }
...
{
    WOLFHSM_CFG_PRINTF("Testing crypto: (pthread) mem...\n");
    WH_TEST_RETURN_ON_FAIL(
        wh_ClientServer_MemThreadTest(WH_NVM_TEST_BACKEND_FLASH));

#if defined(WOLFHSM_CFG_SERVER_NVM_FLASH_LOG)
    WOLFHSM_CFG_PRINTF("Testing crypto: (pthread) mem (flash log)...\n");
    WH_TEST_RETURN_ON_FAIL(
