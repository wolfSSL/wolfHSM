...
#define WH_TEST_RNG_BIG (WOLFHSM_CFG_COMM_DATA_LEN * 2)
    ...
    if (ret != 0) {
        ...
        if (ret != 0) {
             ...
             * timing */
            if (ctx->comm->cryptimeout_enabled == 1)
                serverDelay = 1;
#endif
...
int whTest_CryptoClientConfig_Timeout(whClientConfig* config)
{
    ...
    /* configure time out */
    config->comm->cryptimeout_enabled   = 1;
    ...
    if (ret == 0) {
        ...
        if (ret == WH_ERROR_CRYPTIMEOUT) {
            printf("RNG TIMEOUT TEST SUCCESS SUCCESS\n");
            ret = 0;
            (void)wh_Client_CommClose(client);
        }
        else {
            WH_ERROR_PRINT("RNG TIMEOUT TEST SUCCESS FAILED %d\n", ret);
            if (ret == WH_ERROR_OK) {
                ret = -1; /* force fail */
            }
        }
