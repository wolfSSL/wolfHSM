...
static void _HexDump(const char* p, size_t data_len)
{
    ...
    WOLFHSM_CFG_PRINTF("    HD:%p for %lu bytes\n", p, data_len);
    ...
    for (off = 0; off < data_len; off++) {
        if ((off % bytesPerLine) == 0)
            WOLFHSM_CFG_PRINTF("    ");
        if (u[off] < two_digits) {
            WOLFHSM_CFG_PRINTF("0%X ", u[off]);
        }
        else {
            WOLFHSM_CFG_PRINTF("%X ", u[off]);
        }
...
static void _ShowAvailable(const whNvmCb* cb, void* context)
{
    ...
    rc = cb->GetAvailable(context, &free_space, &free_objects, &reclaim_space,
                          &reclaim_objects);
    if (rc == 0) {
        WOLFHSM_CFG_PRINTF("NVM %p has %u bytes, and %u objects available \n"
        ...
        WOLFHSM_CFG_PRINTF("NVM %p failed to get available info: %d.\n", context, rc);
    }
}
...
static void _ShowList(const whNvmCb* cb, void* context)
{
    ...
    do {
        ...
        if ((rc == 0) && (listCount > 0)) {
            WOLFHSM_CFG_PRINTF("Found object id 0x%X (%d) with %d more objects\n", id, id,
                   listCount - 1);
...
int whTest_NvmFlashCfg(void* cfg, void* context, const whNvmCb* cb)
{
...
#if defined(WOLFHSM_CFG_TEST_VERBOSE)
    WOLFHSM_CFG_PRINTF("--Initial NVM contents\n");
     ...
     * written */
    WOLFHSM_CFG_PRINTF("--Adding 3 new objects\n");
    ...
    WOLFHSM_CFG_PRINTF("--Overwrite an existing object\n");
    ...
    WOLFHSM_CFG_PRINTF("--Overwrite an existing object again \n");
    ...
    WOLFHSM_CFG_PRINTF("--Overwrite an existing object with new data\n");
    ...
    /* Regenerate */
    WOLFHSM_CFG_PRINTF("--Reclaim space\n");
    ...
    /* Ensure reclamation doesn't destroy active objects */
    {
        ...
        unsigned char dataBuf[256];
        size_t i = 0;
        WOLFHSM_CFG_PRINTF("--Read IDs after reclaim\n");
    ...
    /* Destroy 1 object */
    WOLFHSM_CFG_PRINTF("--Destroy 1 object\n");
    ...
    WOLFHSM_CFG_PRINTF("--Destroy 3 objects\n");
...
int whTest_NvmFlash_Recovery(void)
{
    ...
    WOLFHSM_CFG_PRINTF("--simulate failure when writing object start\n");
    ...
    WOLFHSM_CFG_PRINTF("--simulate failure when writing object count\n");
    WH_TEST_RETURN_ON_FAIL(simulateFailureAndRecover(
...
{
    WOLFHSM_CFG_PRINTF("Testing NVM flash with RAM sim...\n");
    WH_TEST_ASSERT(0 == whTest_NvmFlash_RamSim());

    WOLFHSM_CFG_PRINTF("Testing NVM flash recovery mechanism...\n");
    WH_TEST_ASSERT(0 == whTest_NvmFlash_Recovery());

#if defined(WOLFHSM_CFG_TEST_POSIX)
    WOLFHSM_CFG_PRINTF("Testing NVM flash with POSIX file sim...\n");
