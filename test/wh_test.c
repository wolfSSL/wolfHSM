...
>>>>>>> c6f0c63 (attempt 1, failing with no SO_NOSIGPIPE found and implicit declaration of pread)
...
static void* _whClientTask(void *cf)
{
    ...
    /* register the wolfHSM crypto callback */
    if (wc_CryptoCb_RegisterDevice(myDevId, wolfHSM_CryptoCb, ctx) != 0)
        return NULL;
    ...
    /* test rng */
    if((ret = wc_InitRng_ex(rng, NULL, myDevId)) != 0) {
...
static void* _whServerTask(void* cf)
{
    ...
    if (ret == 0) {
        ret = wh_Server_HandleRequestMessage(server);
        if (ret != 0)
            printf("Server failed during rng test\n");
    }
